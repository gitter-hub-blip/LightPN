...
| 51821(默认,面板可改) | UDP | 入站 | 直连 WG(设备直连出口,`lightpn1`),仅在面板启用后监听 | 启用时放行,公网可达 |
...
除了组网用的 `lightpn0`,每个 agent 还能按需开出**第二个 WireGuard 接口 `lightpn1`**:你的手机/电脑直接连它,并以该节点为全隧道出口上网。它与组网数据面完全独立,由 hub 集中管理,**默认关闭**。

面板操作(节点详情页「直连 WG」区):

1. **启用**:选择 UDP 监听端口(默认 51821,勿与组网 WG 端口相同),agent 随即创建 `lightpn1`、生成并**持久化**服务端密钥,并自动配置 IP 转发与 NAT(masquerade 到默认路由网卡;停用/卸载时清理)。记得在该节点防火墙放行所选的 `N/udp`。
2. **添加设备**:浏览器本地生成设备密钥对,面板直接给出可导入 WireGuard 客户端的完整配置(含私钥,**仅此一次展示**,面板与 hub 均不保存);浏览器不支持时可改为粘贴设备公钥。设备地址从该节点的直连网段(默认 `10.99.0.0/24`,与 overlay 网段无关)顺序分配。
3. **移除设备 / 停用**:立即下发 agent 生效;停用只拆接口,设备列表保留,重新启用即恢复。

与组网 WG 的关键差异:直连 WG 的**服务端密钥持久化**在 agent 数据目录(否则你设备里的配置会因 agent 重启而作废),期望状态也在 agent 本地留有副本 —— agent 重启时即使 hub 不在线,你的出口也能自行恢复;hub 恢复后以 hub 侧配置为准对账。这是「边缘零连接记录」的一个刻意例外,只覆盖你自己的设备清单,不涉及组网 peer。

要求与提醒:节点须为 Linux(内核 WG);systemd 单元需含 `CAP_NET_RAW`(新版部署脚本已带,老安装重跑「安装/升级」即可);客户端配置走面板明文经 Cloudflare,信任模型同上一节。
