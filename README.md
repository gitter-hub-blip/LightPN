...
- **翻墙软件配置**:以**服务条**呈现,每个登记的服务一根横向条 —— 左侧显示名与运行状态,右侧「启动|重启|停止」,最右侧 🔒。点锁输入该节点的**查看密码**,浏览器本地解密后锁变箭头,点箭头下滑展开完整配置全文;鼠标移出文本区自动折叠,再点箭头即可重新展开(会话内不重复询问密码)。内置常见路径自动探测到、但未登记成服务的配置文件(xray / sing-box / v2ray / hysteria / trojan-go / shadowsocks-rust / mihomo / clash / caddy / naiveproxy 的标准安装路径)同样显示为条,但状态与操作置灰并标注「未登记,仅可查看」。

文件来源只有两处:内嵌白名单自动探测 + 你在该节点上登记过配置路径的服务(登记项为 unit + 别名 + 可选配置路径,见下节;内置探测覆盖不到的软件靠登记变相兼容)。两者都只存在 agent 本地,hub 无法指定任意路径,故 hub 被攻陷也不能借此读取边缘机器的任意文件。登记路径读取失败时条上会显示具体错误(路径不存在/权限不足),便于排查。

### 配置查看密码(端到端加密,查看配置的前提)

在 agent 部署脚本菜单选「配置查看密码」,或直接执行 `lightpn-agent set-view-pass`,为该节点设置查看密码。**没有查看密码的节点,配置完全不可查看**(agent 只回传 WG 摘要),远程开关服务也不激活。设置后:

- agent 外层只回传**文件元数据**(工具/别名、路径、大小、读取错误 —— 供面板画服务条)与服务状态;**配置内容的每一个字节都只存在于密文里**:以查看密码经 Argon2id(64 MiB/3 轮)派生的密钥做 AES-256-GCM 加密。hub 与 Cloudflare 全程既看不到内容也看不到结构,密码与密钥不经过任何服务器。
- 解密在**浏览器本地**进行(Argon2 WASM + WebCrypto),同一浏览器会话内不重复询问;解锁后展开的是完整明文(自动折叠防旁人窥屏)。
- Argon2 派生只在设密码时(agent 侧一次)与解锁时(浏览器侧)发生,`conf_get` 本身无额外内存开销。
- 密码错误表现为解密校验失败并重新提示;忘记密码在机器上重新 `set-view-pass` 即可(密钥材料 `view.key` 只存该 agent 本地,0600)。`clear-view-pass` 清除密码,同时使面板无法再查看该节点配置。
...
| 事项 | 说明 |
|---|---|
| 备份 | hub 管理脚本菜单「3. 备份数据」一键打包数据目录(SQLite + CA 密钥)与配置为带时间戳的 tar.gz(权限 600,含 CA 私钥请妥善保管)。还原:解包到 APP_DIR 即可。**CA 丢失 = 全网重新入网** |
