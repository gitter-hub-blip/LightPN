...
在 agent 部署脚本菜单选「配置查看密码」,或直接执行 `lightpn-agent set-view-pass`,为该节点设置一个查看密码。设置后:

- agent 回传**打码预览 + 加密全文**:预览里敏感值(私钥/密码/UUID 等,按内嵌正则识别)替换为 ••,结构原样;完整配置以查看密码经 Argon2id(64 MiB/3 轮)派生的密钥做 AES-256-GCM 加密随行。**敏感值 hub 与 Cloudflare 全程只见密文**,密码与密钥不经过任何服务器。
- 面板拉取后直接显示打码预览,**点击任一打码位**才弹密码框,在**浏览器本地**派生密钥并解密(Argon2 WASM + WebCrypto);同一浏览器会话内不重复询问。
- 边界要清楚:预览的非敏感部分(服务器地址、端口、结构)仍是明文过 hub/CDN;打码靠正则识别,未命中的非常规字段名会留在预览明文里。要求"任何内容都不给 CDN 看"的话,请勿在面板拉取配置,改用 SSH。
- Argon2 派生只在设密码时(agent 侧一次)与查看时(浏览器侧)发生,`conf_get` 本身无额外内存开销。
